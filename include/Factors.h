...
struct globalTError
{
	globalTError(double t_x, double t_y, double t_z,
	...
	bool operator()(const T* const q_i, const T* t_i, T* residuals) const
	{
		...
		T q_to_be_opted[4];
		q_to_be_opted[0] = T(q_w);
		q_to_be_opted[1] = T(q_x);
		q_to_be_opted[2] = T(q_y);
		q_to_be_opted[3] = T(q_z);

		T q_to_be_opted_inv[4];
		QuaternionInverse(q_to_be_opted, q_to_be_opted_inv);

		T error_q[4];
		ceres::QuaternionProduct(q_to_be_opted_inv, q_i, error_q); 

		residuals[3] = T(2) * error_q[1] / T(q_var); // just ignore the imaginary part due to normalization
